...
class AboutBudgetScreen extends StatefulWidget {
  final String setPostSufixAndID;
  final String title;

  const AboutBudgetScreen({Key key, this.setPostSufixAndID, this.title})
      : super(key: key);
  @override
  _AboutBudgetScreenState createState() => _AboutBudgetScreenState();
}

class _AboutBudgetScreenState extends State<AboutBudgetScreen> {
  String _budgetItem = '';
  var _isLoaded = false;
  var _isInit = true;
  ...
  void didChangeDependencies() {
    if (_isInit) {
      ...
      final providerBudget = Provider.of<ModelBudget>(context);
      _loadData(context, providerData.fetchPostByID, widget.setPostSufixAndID,
              providerBudget.setpostsBudget)
          .then((value) {
        providerBudget.fetchBudget();
        _budgetItem = providerBudget.budget;
      });
    }
    _isInit = false;
